...
-- 1. 查询每个月倒数第 2 天入职的员工的信息.
    ...
    where hire_date = last_day(hire_date)-1  --last_day() 返回给定日期所在的月底日期 
-- 2. 查询出 last_name 为 'Chen' 的 manager 的信息.
    -- 1). 通过两条 sql 查询: 通过'chen'查询manager的id，得到manager信息
        select manager_id
        from employees
        where lower(last_name) = 'chen' -- lower()小写转换  返回的结果为 7566
    ...
    -- 2). 通过一条 sql 查询(自连接):
    select m.*
    from employees e, employees m   -- 内连接
    where e.manager_id = m.employee_id and e.last_name = 'Chen'  
    --3).通过一条 sql 子查询 
    select m.* 
    from employees m
    where employee_id=(select e.manager_id from employees e where e.last_name='Chen')
    -- 3). 通过一条 sql 查询(子查询):
    select *
    from employees
    where employee_id = (
...
-- 3. 查询平均工资高于 8000 的部门 id 和它的平均工资.
    ...
    SELECT department_id, avg(salary)   -- avg()平均值
    FROM employees e
    GROUP BY department_id              --根据部门id分组,求部门工资平均值
    HAVING avg(salary) > 8000           --过滤条件：平均工资超过8000的部门  聚合函数过滤应当是在WHERE之后进行的
-- 4. 查询工资最低的员工信息: last_name, salary
    SELECT last_name, salary
    FROM employees
    WHERE salary = (
        SELECT min(salary)   -- min()最小值
        FROM employees
    )
-- 5. 查询平均工资最低的部门信息
    SELECT *
...
-- 12. 查询 1999 年来公司的人所有员工的最高工资的那个员工的信息.
    ...
    SELECT *
    FROM employees
    WHEREto_char(hire_date,'yyyy')='1999'ANDsalary
    = (
    SELECT max(salary)
    FROM employees
    WHERE to_char(hire_date, 'yyyy') = '1999'
